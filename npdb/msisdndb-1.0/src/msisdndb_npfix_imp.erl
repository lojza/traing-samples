%%
%% NPDB FIX imort module
%%

-module(msisdndb_npfix_imp).
...
process_line (BinLine, _LineNum, #st{counter = N, split_patt = Patt, values = Values} = St) ->
    if (N rem 100000 == 0) and (N > 0) -> ?LOG_INFO ("NPFIX import records: ~p", [N]);
       true -> ok
    end, 
    ...
    %msisdndb_npfix:insert_async (M1, M2, OpId),
    Data = {M1, M2, OpId},
    
    {ok, St#st{counter = N + 1, values = [Data|Values]}}.

make_result(#st{counter = N, values = Values}) ->
    ?LOG_INFO ("NPFIX import records: ~p", [N]),
    case msisdndb_npfix:insert_many (Values) of
      ok -> {ok, N};
      Any -> Any
    end.
