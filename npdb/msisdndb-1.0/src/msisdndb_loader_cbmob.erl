-module(msisdndb_loader_cbmob).

-export([init/0, file_type/2]).

init () ->
  {ok, ReIncr} = re:compile("^MOBNPDB(\\d{4})(\\d{2})(\\d{2})-(\\d{2})(\\d{2})\\.txt$"),
  {ok, ReFull} = re:compile("^mob_resync_(\\d{4})_(\\d{2})_(\\d{2})_(\\d{2})_(\\d{2})\\_(\\d{2}).csv$"),
  {ReIncr, ReFull}.
